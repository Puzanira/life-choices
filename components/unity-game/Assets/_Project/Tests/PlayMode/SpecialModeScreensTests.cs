...
namespace ThanksNoThanks.Tests.PlayMode
{
    ...
    public class SpecialModeScreensTests
    {
        ...
        public IEnumerator PlatePunch_OnlyOnAnAcceptedAnswer_NotInAnySwallowedState()
        {
            ...
            fake.Yes();
            yield return null;
            // ⚠ ПАНЧ ТЕПЕРЬ SQUASH (r4, «по желанию» дизайн-гейта): в нижней точке дуги плашка ШИРЕ
            // и НИЖЕ обычного, а не равномерно меньше. Признак отклика — просадка по ВЫСОТЕ; по
            // ширине сравнение «<1» стало бы вечно красным, хотя отклик на месте. Предмет теста
            // (панч играет ТОЛЬКО на принятый ввод) не тронут — форма дуги стережётся отдельным
            // гардом PlatePunch_SquashesThePlate_RatherThanShrinkingIt.
            Assert.Less(yes.localScale.y, 1f, "принятый ответ → плашка панчит");
        ...
        /// <summary>
        /// ⚠ ТЕСТ ПЕРЕСПЕЦИФИЦИРОВАН 2026-08-08 (r4 п.4) — РАНЬШЕ ОН УТВЕРЖДАЛ ОБРАТНОЕ.
        ///
        /// Было: «в блице плашка не панчит — это не ответ». Рассуждение r2/r3 звучало так: в блице рычаги
        /// ПЕРЕНАЗНАЧЕНЫ (это кнопки блица, а не ДА/НЕТ по карточке), значит и обратной связи «ответ
        /// засчитан» быть не должно.
        ///
        /// Стало: основательница на живом плейтесте назвала ровно это дефектом — «кнопки блица в старом
        /// стиле и НЕ АНИМИРОВАНЫ». С её стороны экрана панч читается не как «ответ на карточку принят»,
        /// а как «железо меня услышало»: если по всей игре нажатие роняет плашку, а в самом быстром
        /// режиме игры — нет, блиц выглядит залипшим.
        ///
        /// Что при этом НЕ меняется — accepted-семантика r2/r3: панч по-прежнему играет ТОЛЬКО на
        /// ПРИНЯТЫЙ ввод. Просто «принято» в блице означает «рычаг засчитан как нажатие по живой мысли»
        /// (BlitzPress принимает оба рычага всегда), а не «ответ на карточку». Все состояния, где игра
        /// ввод ГЛУШИТ, остаются немыми — их стережёт соседний тест
        /// <see cref="PlatePunch_OnlyOnAnAcceptedAnswer_NotInAnySwallowedState"/>.
        ///
        /// ⚠ ЧЕСТНОСТЬ ИМЕНИ. Старое имя обещало ещё и ДЕПРЕССИЮ, но ТЕЛО теста в неё никогда не заходило:
        /// прогон крутился до `CrisisPhase.Blitz` и на нём же заканчивался — половина названия была
        /// пустой. Заводить депрессию отсюда нечем (отладочного входа в неё нет, в отличие от кризиса),
        /// поэтому имя сужено до того, что тест ДЕЙСТВИТЕЛЬНО проверяет. Инертность зелёной в депрессии
        /// закрыта на уровне ввода (`Game.HandleInput`: в депрессии живёт только ловля на «!»), а панч
        /// висит на принятии ввода — то есть молчание плашек там следует из уже закрытого инварианта.
        /// </summary>
        [UnityTest]
        public IEnumerator PlatePunch_FiresInTheBlitz_OnAnAcceptedPress()
        {
            var driver = Boot(out var go, out var fake);
            yield return null;
            var g = CrisisGame();
            ...
            // БЛИЦ: рычаг засчитан как нажатие по живой мысли ⇒ плашка ОБЯЗАНА панчить (r4 п.4).
            Assert.AreEqual(CrisisPhase.Blitz, g.Phase, "в блице");
            Assert.AreEqual(1f, yes.localScale.x, 1e-3f, "исходно зелёная плашка в норме");
            fake.Yes();
            yield return null;
            Assert.Less(yes.localScale.y, 1f, "принятое нажатие в блице → зелёная плашка панчит");
            yield return new WaitForSeconds(0.25f);        // дать корутине доиграть
            Assert.AreEqual(1f, yes.localScale.x, 1e-2f, "…и возвращается");

            // Красная — ровно так же: отклик про ПАЛЕЦ игрока, а не про попадание. Промах по «ВСЁ
            // НОРМАЛЬНО» тоже засчитан как нажатие (BlitzPress принимает оба рычага) и тоже панчит;
            // разницу «попал/промазал» несёт ГОЛОС, а не плашка.
            Assert.AreEqual(CrisisPhase.Blitz, g.Phase, "блиц ещё идёт");
            Assert.AreEqual(1f, no.localScale.x, 1e-3f, "исходно красная плашка в норме");
            fake.No();
            yield return null;
            Assert.Less(no.localScale.y, 1f, "…и красная панчит на своё принятое нажатие");
            yield return new WaitForSeconds(0.25f);
            Assert.AreEqual(1f, no.localScale.x, 1e-2f, "…и тоже возвращается");
