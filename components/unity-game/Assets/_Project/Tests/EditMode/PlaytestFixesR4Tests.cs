...
namespace ThanksNoThanks.Tests
{
    ...
    public class PlaytestFixesR4Tests
    {
        ...
        /// а тяга вверх была всего 4.0, нетто +0.8 %/с ≈ «стоит на месте».
        ///
        /// ⚠ ОКНО СОКРАЩЕНО 5 с → 1 с (r5-ревю, находка код-скептика). Прежний замер был ФИКТИВНЫМ: после
        /// разгона тяги 4.0 → 22.0 удержание с отметки 50 упирается в потолок 100 за ~2.5 с, и пятисекундное
        /// окно всегда возвращало ровно (100−50)/5 = 10 %/с — число КЛАМПА, а не скорости. Тест был зелёным
        /// и с полностью удалённым полом. Окно взято таким, чтобы замер не доставал потолка, и это
        /// проверяется явным ассертом, а не на слово (образец — `GameRelationshipsTests.
        /// AxisMagnitude_MatchesTheBalancerConstant`).
        ///
        /// ЧТО ДЕРЖИТ ЭТОТ ГАРД ПОСЛЕ ПРАВКИ: на всех СЕГОДНЯ достижимых множителях (включая стак ×2·×2)
        /// активное удержание ↑ уверенно тянет вверх. На текущем балансе это обеспечивает сама тяга, а не
        /// пол — поэтому тест честно назван про тягу и НЕ выдаёт себя за проверку пола. Собственно пол
        /// проверяет соседний
        /// <see cref="HoldingUp_HitsTheNetFloor_WhenTheDriftOutrunsThePull"/>.
        /// </summary>
        [TestCase(1.0, TestName = "без множителя")]
        [TestCase(2.0, TestName = "MD01=НЕТ (x2, бессрочный)")]
        [TestCase(4.0, TestName = "стак множителей (x4)")]
        public void HoldingUp_BeatsTheDrift_UnderAnyLegalMultiplier(double mult)
        {
            var g = GameWithRelDrift(mult);
            SettleDownTo(g, 50);                         // запас хода вверх, вдали от порога разрыва

            double rate = HoldRate(g, +1, 1f);
            Assert.Less(g.Scales.Relationships, 100,
                "замер не упёрся в потолок — иначе мерили бы кламп шкалы, а не скорость");
        ...
        /// <summary>
        /// СОБСТВЕННО ПОЛ УДЕРЖАНИЯ — на режиме, где он РЕАЛЬНО СРАБАТЫВАЕТ.
        ///
        /// После r5 (тяга 22.0) пол <see cref="Game.RelHoldNetFloorPerSec"/> спит: чтобы он вмешался, дрейф
        /// должен обогнать тягу, то есть множитель перевалить за ×12.5 — в живой колоде такого нет. Пол
        /// оставлен СТРАХОВКОЙ на класс поломки «стак множителей съел удержание» (ровно тот, что поймала
        /// основательница в r4), и раз он оставлен — он обязан быть покрыт ЧЕСТНО, а не тестом, который
        /// зелен и без него.
        ///
        /// Множитель здесь СИНТЕТИЧЕСКИЙ (×16, дрейф 25.6 %/с против тяги 22.0) — он сознательно выходит за
        /// сегодняшнюю колоду и моделирует завтрашнюю. Оба предусловия проверяются ассертами: сырое нетто
        /// ОТРИЦАТЕЛЬНО и лежит НИЖЕ пола, значит измеряется именно пол, а не остаток тяги.
        ///
        /// Mutation-proof: удали пол (или сделай его безусловным/нулевым) — удержание поедет вниз со
        /// скоростью −3.6 %/с вместо +2.0, тест краснеет с запасом в пять с лишним процентов в секунду.
        /// </summary>
        [Test]
        public void HoldingUp_HitsTheNetFloor_WhenTheDriftOutrunsThePull()
        {
            const float window = 5f;   // пол даёт всего 2 %/с — окну нужна длина, чтобы целые шаги набежали
            const double synthetic = 16.0;

            var g = GameWithRelDrift(synthetic);

            Assert.Greater(g.RelationshipDriftPerSec, Game.RelBalancerPerSec,
                "предусловие: дрейф обгоняет тягу — БЕЗ пола удержание ↑ уводило бы шкалу ВНИЗ");
            Assert.Less(Game.RelBalancerPerSec - g.RelationshipDriftPerSec, Game.RelHoldNetFloorPerSec,
                "предусловие: сырое нетто ниже пола — значит замер меряет ИМЕННО пол");

            SettleDownTo(g, 50);                         // ход вверх есть, до порога разрыва далеко

            double rate = HoldRate(g, +1, window);
            Assert.Less(g.Scales.Relationships, 100,
                "замер не упёрся в потолок — иначе мерили бы кламп шкалы");

            // ГЛАВНОЕ УТВЕРЖДЕНИЕ, и оно качественное: со включённым полом удержание тянет ВВЕРХ, хотя
            // дрейф обгоняет тягу. Без пола тут было бы −3.6 %/с — промах не на допуск, а на знак.
            Assert.Greater(rate, 0.0,
                $"при дрейфе {g.RelationshipDriftPerSec:F1} %/с против тяги {Game.RelBalancerPerSec} "
                + $"удержание ↑ обязано ТЯНУТЬ ВВЕРХ, а не вниз (замерено {rate:F2} %/с)");

            // …и количественное: отыгрывается именно ПОЛ. Допуск — ровно ОДНО целое деление на окно
            // (шкала ходит целыми, стартовый остаток аккумулятора может съесть последний шаг) плюс
            // эпсилон на двоичное представление: замер 9 шагов / 5 с даёт 1.7999999…, а не ровно 1.8.
            // Эпсилон именно 1e-6, а не машинный: окно копится как float (0.05f не представимо точно),
            // поэтому 100 тиков дают 5.0000000745 с, и частное промахивается мимо 1.8 на ~3e-8.
            const double quantum = 1.0 / window, eps = 1e-6;
            Assert.GreaterOrEqual(rate, Game.RelHoldNetFloorPerSec - quantum - eps,
                $"при дрейфе {g.RelationshipDriftPerSec:F1} %/с удержание ↑ обязано отыгрывать пол "
                + $"{Game.RelHoldNetFloorPerSec} %/с нетто (замерено {rate:F2})");
        }
