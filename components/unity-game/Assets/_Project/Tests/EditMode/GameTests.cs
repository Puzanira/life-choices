...
namespace ThanksNoThanks.Tests
{
    public class GameTests
    {
        ...
        public void FullRun_AllNo_ReachesNaturalEnding_WithAssembledNecrolog()
        {
            ...
            //
            // ⚠ ПЕРЕСПЕК r4 п.2: раньше здесь стоял ИНДЕКС (`StoryLines[1]`), и это делало гард
            // заложником РАЗМЕРА колоды. `YA03` выброшена решением основательницы, спина стала на одну
            // карточку короче — освободился слот, в него законно прошла более РАННЯЯ детская
            // ROND-строка, и она встала первой при сортировке по возрасту. Механика отбора при этом
            // цела: весомая строка на месте. Проверяем присутствие, а не позицию — инвариант тот же,
            // а к длине колоды гард больше не привязан.
            CollectionAssert.Contains(n.StoryLines,
                "В детстве чуть не тронули розетку — но вовремя одумались.",
                "весомая детская строка обязана попасть в некролог (отбор по ВЕСУ, не по порядку)");
