...
namespace ThanksNoThanks
{
    ...
    public sealed class GameDriver : MonoBehaviour
    {
        ...
        // Рект САМОГО текста: бокс, ужатый слева на поле выше (центр уезжает на половину инсета).
        private static readonly Vector4 FinaleStoryRect = new(
            FinaleStoryBox.x + FinaleStoryLeftInset / 2f, FinaleStoryBox.y,
            FinaleStoryBox.z - FinaleStoryLeftInset, FinaleStoryBox.w);     // x 415…1511
        ...
        private void BuildFinale(Transform parent)
        {
            ...
            // строки. Строка ИСХОДА (`_finaleOutcome`) осталась ПО ЦЕНТРУ: это отдельный Text и отдельный
            // блок, её основательница не трогала.
