...
namespace ThanksNoThanks
{
    ...
    public sealed class GameDriver : MonoBehaviour
    {
        ...
        private Text _finaleOutcome;   // «Ты дожил до N лет» + «Причина конца: …», Rubik-Bold, по левому краю
        ...
        // ⚠ РЕКТ СТРОКИ ИСХОДА объявлен НИЖЕ (<see cref="FinaleOutcomeRect"/>), рядом с ректом некролога:
        // с 2026-09-22 оба выводятся из ОДНОЙ колонки, а инициализаторы static-полей C# исполняются в
        // текстовом порядке — колонка обязана быть объявлена раньше обоих ректов.
        ...
        /// </summary>
        public static readonly Vector4 FinaleStoryBox = new(959f, 664f, 1104f, 314f);   // x 407…1511, y 507…821

        /// <summary>
        /// ЛЕВОЕ ПОЛЕ БЛОКА — появилось вместе с выравниванием по левому краю (решение основательницы
        /// 2026-09-22: «выравнивание надо по левому краю, а не по центру»).
        ///
        /// ⚠ ЗАЧЕМ ОНО ВООБЩЕ НУЖНО. При центровке левая кромка ГЛИФОВ никогда не доходила до кромки ректа:
        /// поле <see cref="FinaleStorySideMargin"/> сидело в бюджете ширины строки и раздавалось поровну на
        /// две стороны. Левое выравнивание сажает КАЖДУЮ строку ровно на левую кромку ректа — то есть на
        /// x 407, в четырёх пикселях от запечённой ЗВЕЗДЫ (x ≤ 403 на y 474…556, с этой полосой она
        /// перекрывается первыми 49 px). Четыре пикселя между глифом и звездой — это «на звезде» на глаз,
        /// а гард бокса такого не видит: он судит РЕКТ, а рект как стоял, так и стоит.
        ///
        /// Поэтому рект текста ужимается СЛЕВА ровно на то поле, которое и так держалось по бокам: левая
        /// кромка строк встаёт на 415, зазор до звезды 12 px, правая кромка ректа не двигается. Безопасный
        /// бокс <see cref="FinaleStoryBox"/> при этом НЕ ТРОНУТ — он и остаётся приёмкой «полоса на креме и
        /// не на выкусе», а рект текста теперь строго внутри него.
        /// </summary>
        public const float FinaleStoryLeftInset = FinaleStorySideMargin;

        // ================================================================= ОДНА КОЛОНКА НА ВЕСЬ ФИНАЛ
        //
        // ⚠ РЕШЕНИЕ ОСНОВАТЕЛЬНИЦЫ 2026-09-22 (вторая итерация, дословно): «причина конца и ты дожил тоже
        // надо по левому краю. плюс шрифт текста я другой выбрала же». То есть ЗАГОЛОВОК финала («Ты дожил
        // до N лет» / «Причина конца: …») догоняет некролог по ОБОИМ признакам: левое выравнивание и то же
        // лицо (Rubik-Bold, вариант C).
        //
        // ПОЧЕМУ КОЛОНКА, А НЕ ВТОРОЕ ЧИСЛО. Левое выравнивание двух блоков читается только если кромка у
        // них ОДНА. Держать её двумя независимыми литералами — значит завести расхождение, которое никто
        // не заметит: сдвинули один рект, второй остался, и заголовок «почти» над списком. Поэтому кромки
        // выводятся из одной пары чисел ниже, и оба ректа — из неё.
        /// <summary>Левая кромка ОБОИХ текстовых блоков финала: 407 (левый край полосы) + поле 8 = 415.</summary>
        public static readonly float FinaleTextColumnLeft =
            FinaleStoryBox.x - FinaleStoryBox.z / 2f + FinaleStoryLeftInset;
        /// <summary>Правая кромка колонки — правый край полосы некролога (1511), он же потолок ширины.</summary>
        public static readonly float FinaleTextColumnRight = FinaleStoryBox.x + FinaleStoryBox.z / 2f;
        private static readonly float FinaleTextColumnCx =
            (FinaleTextColumnLeft + FinaleTextColumnRight) / 2f;                     // 963
        private static readonly float FinaleTextColumnW =
            FinaleTextColumnRight - FinaleTextColumnLeft;                            // 1096

        // Рект САМОГО текста: та же колонка, вертикаль — от бокса некролога. Числа не изменились
        // (центр 963, ширина 1096, x 415…1511) — изменился только ИСТОЧНИК: колонка, а не свой расчёт.
        private static readonly Vector4 FinaleStoryRect = new(
            FinaleTextColumnCx, FinaleStoryBox.y, FinaleTextColumnW, FinaleStoryBox.w);   // x 415…1511

        /// <summary>
        /// Безопасный бокс ПОЛОСЫ ИСХОДА (y 330…452) — ровно колонка, не шире.
        ///
        /// Полосе исхода не мешает НИ ОДНА из трёх запечённых звёзд: левая живёт на y 474…556, правые две —
        /// на y 833…865, то есть обе ниже полосы целиком. Формально ширину можно было бы раздать до кромки
        /// крема (x 342.7…1575.2). НЕ РАЗДАЁМ НАМЕРЕННО: заголовок теперь выровнен по левому краю вместе с
        /// некрологом, и любая своя кромка у него — это две разные вертикали на одном экране. Колонка
        /// общая; лишний крем по бокам — поля, а не резерв.
        /// </summary>
        public static readonly Vector4 FinaleHeadBox = new(
            FinaleTextColumnCx, 391f, FinaleTextColumnW, 122f);             // x 415…1511, y 330…452

        // Рект строки исхода = её бокс (полей внутри полосы нет: блок и так прижат к левой кромке колонки,
        // а правая кромка для двух коротких строк ничего не режет).
        private static readonly Vector4 FinaleOutcomeRect = FinaleHeadBox;   // y 330…452

        /// <summary>
        /// Кегли строки исхода (с 2026-09-22 — Rubik-Bold, лицо некролога): верх — с эталона, низ — предел
        /// ужатия.
        ///
        /// ПОТОЛОК 56 СОХРАНЁН ПРИ СМЕНЕ ЛИЦА, и подбирать его вручную не пришлось — замер на отдаваемом
        /// дереве (гард <c>Finale_OutcomeHead_OnTheNecrologKerb_SameFace_CauseNeverWraps</c>, все 12 причин
        /// колоды):
        ///   · обычная причина       → best-fit берёт 51;
        ///   · худшая причина колоды → 49 («Причина конца: вы сунули палец в розетку», 40 знаков).
        /// То есть 56 не достигается НИКОГДА, и ограничивает не он: две строки заголовка в полосе высотой
        /// 122 px упираются в ВЫСОТУ на 51, а худшая причина добирает ещё −2 по ШИРИНЕ колонки. Потолок
        /// остаётся запасом на случай, если строки станут короче, — трогать его незачем.
        ///
        /// ⚠ ПЕРЕНОСА ВНУТРИ ПРИЧИНЫ НЕТ НИ ПРИ ОДНОЙ ПРИЧИНЕ, и это держит гард, а не расчёт: best-fit с
        /// переносом слов «влезает» и в три ряда, просто мельче, поэтому проверка на два ряда обязательна.
        ///
        /// ИЕРАРХИЯ: заголовок 49…51 против 30 у списка (вариант C) — на общем лице разницу даёт кегль.
        /// </summary>
        ...
        /// <summary>
        /// НАБОР, КОТОРЫЙ СТОИТ В ИГРЕ — «C», и это больше НЕ рекомендация, а КАНОН.
        ///
        /// ✔ РЕШЕНИЕ ОСНОВАТЕЛЬНИЦЫ 2026-09-22 (живой отсмотр кадров r5-finale-A/B/C.png), дословно:
        /// «Лучше всего вариант C». Выбор закрыт.
        ///
        /// Переключатель A/B/C (<see cref="FinaleStoryStyles"/> + позы `finalea`/`finaleb`/`finalec`)
        /// ОСТАВЛЕН НАМЕРЕННО и стоит ровно одну строку — это готовая вилка для следующего разговора о
        /// типографике финала. Но по умолчанию игра показывает C, и сменить его без нового слова
        /// основательницы нельзя.
        /// </summary>
        ...
        private void BuildFinale(Transform parent)
        {
            ...
            // (2) Строка исхода — В запечённую плашку, вверх её кремового поля. INK (канон §12-4), БЕЗ
            // DisplayFx: тёмная обводка по тёмным глифам на светлом креме только мажет их в кляксу (та же
            // причина, что у вопроса карточки).
            //
            // ЛИЦО: `_bodyBold` = Rubik-Bold — ТОТ ЖЕ АССЕТ, что у некролога.
            // ⚠ Было `_display` (Arimo Bold): заголовок и список под ним набирались РАЗНЫМИ гарнитурами,
            // и после того как некролог переехал на Rubik-Bold (вариант C, выбор основательницы), стык
            // стал виден в упор. Её слово 2026-09-22: «шрифт текста я другой выбрала же».
            //
            // ВЫРАВНИВАНИЕ: `UpperLeft` — по ТОЙ ЖЕ левой кромке, что некролог (колонка
            // <see cref="FinaleTextColumnLeft"/>, x 415). Было `UpperCenter`. Её слово там же: «причина
            // конца и ты дожил тоже надо по левому краю». Обе строки заголовка разной длины, и центровка
            // давала им разный отступ — ровно та же болезнь, от которой лечили список ниже.
            _finaleOutcome = NewText("FinaleOutcome", _finalePanel.transform,
                "", FinaleOutcomeMaxSize, TextAnchor.UpperLeft, Ink, _bodyBold);
            ...
            // (3) Некролог — туда же, под строкой исхода. Rubik, INK.
            //
            // ВЫРАВНИВАНИЕ: ПО ЛЕВОМУ КРАЮ, по вертикали — по центру бокса (`MiddleLeft`).
            // ⚠ 2026-09-22, решение основательницы по живым кадрам: «выравнивание надо по левому краю, а не
            // по центру». Центровка ставила каждой строке свой отступ слева (строки-то разной длины), и
            // список из семи законченных предложений читался рваным — глазу негде зацепиться за начало
            // строки. ⚠ ВТОРОЙ ИТЕРАЦИЕЙ ТОГО ЖЕ ДНЯ строка ИСХОДА (`_finaleOutcome`) переехала на ту же
            // кромку и то же лицо — «причина конца и ты дожил тоже надо по левому краю»; кромка у обоих
            // блоков одна (<see cref="FinaleTextColumnLeft"/>), а не два совпавших числа.
            // По вертикали центр сохранён намеренно: короткая FATAL-история встаёт в середину поля, а не
            // жмётся к строке исхода, — и ровно сюда же уходит высота, освободившаяся от снятой подводки.
            _finaleStory = NewText("FinaleStory", _finalePanel.transform,
                "", FinaleStoryMaxSize, TextAnchor.MiddleLeft, Ink, _body);
