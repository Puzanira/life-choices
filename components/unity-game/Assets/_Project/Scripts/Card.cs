...
namespace ThanksNoThanks
{
    ...
    public enum LongEffectKind
    {
        ...
        /// <summary>
        /// РАЗОВАЯ выплата через N игровых лет (<c>DRAIN:Дн=-25 ONCE:3y</c>) — отрезки 1–7. Не дренаж:
        /// один удар по счёту в назначенный год. Ровно то, что обещает проза трёх карточек — «через ~3
        /// года долг придётся закрывать самому» (`FA27`), «через ~5 лет чужой долг становится твоим»
        /// (`FB32`), «через ~10 лет вернётся вдвое» (`FC32`, единственная ПОЛОЖИТЕЛЬНАЯ). Дренажом это
        /// не выразить: −25 ₽ размазанные по годам ощущаются как ничто, а карточка обещает удар.
        /// </summary>
        Once
    }
    ...
    public struct LongEffect
    {
        ...
        // --- Once ---
        /// <summary>Разовая сумма в ₽ (знаковая) для <see cref="LongEffectKind.Once"/>.</summary>
        public double OnceAmount;
        /// <summary>Через сколько игровых лет от ответа падает разовая сумма (<c>ONCE:3y</c>).</summary>
        public int OnceYears;
    ...
    public sealed class Card
    {
        ...
        /// <summary>
        /// СТОРОНА разрыва. `MD24` «Потерпеть ради ребёнка?» рвёт брак на **НЕТ** («Развод»), а не на ДА —
        /// до отрезка 6 все `BREAK:Отн` жили на ДА, и одной стороны хватало. Формат тот же префикс, что у
        /// «Длительного эффекта»: <c>НЕТ:BREAK:Отн</c> в колонке флагов; без префикса — ДА, как было.
        /// </summary>
        public bool BreakOnNoSide;

        /// <summary>
        /// «Когда» вида «…, до <c>MD01</c>» (`MD08` свадьба на все деньги, `MD09` брачный договор) — карточка
        /// обязана выпасть РАНЬШЕ названной. Возраст ей назначает <see cref="DeckSampler"/> (веха − 1 год):
        /// «до свадьбы» без этого превращается в лотерею, потому что окно 28–32 у обеих одно и то же.
        /// null — обычная карточка.
        /// </summary>
        public string BeforeCardId;

        /// <summary>
        /// «Когда» вида «свадьба +N» (`MD02` ребёнок, `MD11` фамилия, `MD12` годовщина, `MD13` детей не
        /// будет). Возраст = возраст `MD01` + N, и карточка автоматически гейтится на `MD01`=ДА: без
        /// свадьбы годовщины не бывает. −1 — не свадебная карточка.
        /// </summary>
        public int MarriageOffsetYears = -1;

        /// <summary>
        /// «Когда» вида «если на счету &lt; 5 ₽» (`FA35` первый ноль) или «и на счету минус» (`FC16`
        /// вернуться к родителям). Карточка приходит ТОЛЬКО тому, у кого на счету меньше порога — тот,
        /// кто крутил, этой сцены не увидит вовсе. <see cref="double.NaN"/> — условия нет.
        /// </summary>
        public double RequiresMoneyBelow = double.NaN;

        /// <summary>
        /// «Когда» вида «если на счету ≥ 60 ₽» (`MD03` машина, `MD04` поздняя ипотека). Зеркало
        /// <see cref="RequiresMoneyBelow"/>: карточка достатка приходит только тому, у кого достаток есть.
        /// <see cref="double.NaN"/> — условия нет.
        /// </summary>
        public double RequiresMoneyAtLeast = double.NaN;

        /// <summary>
        /// «Когда» вида «если Отн потеряна» (`MD06` второй шанс). Карточка приходит только тому, у кого
        /// отношения УЖЕ порваны — иначе «второй шанс» предлагался бы поверх живого партнёра.
        /// </summary>
        public bool RequiresRelationshipsLost;

        /// <summary>
        /// «Когда» вида «если Отн открыта» / «если шкала отношений открыта» / «если Дн открыта»
        /// (`MD01` свадьба, `FB31`, `RND05`, `YA04`) — токен ШКАЛЫ, которая обязана быть ОТКРЫТА в момент
        /// показа (<see cref="OpenRelations"/> и соседи, тот же словарь, что у `OPEN:*` и Δ-колонки).
        ///
        /// ⚠ НАХОДКА РЕВЮ (MAJOR): до этого форма молча игнорировалась, и «свадьба» приходила игроку,
        /// который на `YA03` ответил НЕТ и никаких отношений не открывал, а следом `MD02` предлагала
        /// ребёнка. null — условия нет.
        /// </summary>
        public string RequiresScaleOpen;

        /// <summary>
        /// «Когда» вида «если в браке» — гейт ПО ЖИВОМУ БРАКУ на момент показа (<see cref="Game.Married"/>).
        ///
        /// ⚠ ЗАЧЕМ ОТДЕЛЬНО ОТ <see cref="RequiresParentYes"/> (r4 п.5, живой плейтест основательницы:
        /// «после расставания приходят карточки про свадьбу»). Вся свадебная ветка держалась на ИСТОРИИ
        /// ОТВЕТОВ: `свадьба +N` и «если MD01=ДА» спрашивают только «игрок когда-то сказал ДА на MD01?».
        /// Развод/разрыв гасит <see cref="Game.Married"/> и закрывает шкалу, но запись `_answers["MD01"]
        /// = true` остаётся НАВСЕГДА — и пак «Брак на износе», годовщины и «взять его фамилию?» продолжали
        /// выпадать человеку, который давно один. История отвечает «что было», живой гейт — «что есть
        /// СЕЙЧАС»; свадебной ветке нужны ОБА.
        ///
        /// НЕ ПЕРЕГИБАТЬ: ребёнок разводом не отменяется. Карточки, гатящиеся на `MD02=ДА` (утренник,
        /// садик, внуки, наследство), живого гейта НЕ получают — они про ребёнка, а не про брак.
        /// </summary>
        public bool RequiresMarried;

        /// <summary>
        /// «Когда» вида «если Здр&lt;50%» (`LT02` операция) / «когда здоровье &lt; 40%» (`LT08` подлечиться).
        /// Раньше условие `LT02` было ЗАШИТО по id прямо в <see cref="Game"/>; теперь читается из CSV, как
        /// и все прочие. <see cref="double.NaN"/> — условия нет.
        /// </summary>
        public double RequiresHealthBelow = double.NaN;

        /// <summary>
        /// «Когда» вида «(возраст ≥ 30)» (`LT08`) — нижняя граница возраста показа. −1 — условия нет.
        /// </summary>
        public int RequiresMinAge = -1;

        /// <summary>
        /// Куски колонки «Когда», которые парсер НЕ РАСПОЗНАЛ. Пустой список — норма.
        ///
        /// ⚠ ЗАЧЕМ ПОЛЕ (находка ревю, MAJOR). Раньше незнакомая формулировка молча игнорировалась, и
        /// целый КЛАСС ошибок («условие написано, но не работает») был невидим: `MD01` без отношений,
        /// `FB31`, `MD03`/`MD04` без достатка. Теперь парсер не «пропускает», а ОТЧИТЫВАЕТСЯ, и валидатор
        /// колоды (`FullDeckConformanceTests`) краснеет на первой же нераспознанной форме. Загрузку это
        /// по-прежнему не роняет — колонка человеческая, но незамеченной опечатка больше не проходит.
        /// </summary>
        public readonly List<string> UnparsedWhen = new();
